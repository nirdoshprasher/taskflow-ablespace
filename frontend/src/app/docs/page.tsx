...
import { ArrowLeft, Plus, Trash2, ChevronDown, ChevronRight, Copy, Check } from 'lucide-react';
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {Object.entries(METHOD_COLORS).map(([m, c]) => (
              <span key={m} style={{ background: c.bg, color: c.text, padding: '2px 10px', borderRadius: 99, fontSize: 11, fontWeight: 700 }}>
                {m}
              </span>
            ))}
          </div>
          ...
          {docs.map(doc => {
            ...
            return (
              <div key={doc.id} style={{
                ...
                >
                  <span style={{ background: mc.bg, color: mc.text, padding: '3px 10px', borderRadius: 6, fontSize: 11, fontWeight: 800, flexShrink: 0, letterSpacing: '0.04em' }}>
                    {doc.method}
                  </span>
                  <code style={{ fontSize: 13, color: 'var(--text-muted)', background: 'var(--bg-secondary)', padding: '2px 10px', borderRadius: 6, flexShrink: 0 }}>
                    {doc.endpoint}
                  </code>
                ...
                {doc.open && (
                  <div style={{ padding: '0 18px 18px', borderTop: '1px solid var(--border-light)' }}>
                    <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 14, marginBottom: 14, lineHeight: 1.6 }}>
                      {doc.description}
                    </p>
                    ...
                    {doc.requestBody && (
                      <CodeBlock
                        label="Request Body"
                        code={doc.requestBody}
                        id={`req-${doc.id}`}
                        copied={copied === `req-${doc.id}`}
                        onCopy={() => copyToClipboard(doc.requestBody!, `req-${doc.id}`)}
                      />
                    )}
                    ...
                    <CodeBlock
                      ...
                      code={doc.response}
                      id={`res-${doc.id}`}
                      copied={copied === `res-${doc.id}`}
                      onCopy={() => copyToClipboard(doc.response, `res-${doc.id}`)}
                    />
                  </div>
                )}
              </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
        ...
        <button
          ...
          style={{
            ...
            fontSize: 11, color: copied ? '#16a34a' : 'var(--text-muted)',
